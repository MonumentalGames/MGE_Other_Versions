...
class Object2D:
    ...
    def draw_object(self, screen, camera=None, pivot=Pivot2D.TOP_LEFT_SIDE):
        ...
        if screen.opengl:
            cache_localization[1] = size_screen[1] - cache_size[1] - cache_localization[1]

        if self.material.texture or 254 >= self.material.alpha >= 0 or self.rotation > 0 or not self.Mesh.Mesh == plane:
            if not self.material.object_render or self.material.always_render or not self.object_render or self.always_render or screen.render_all_objects:
                if screen.opengl:
                    self.material.render(True)
                else:
                ...
                screen.render_all_objects = False
            ...
            if not self.border_size == 0:
                ...
                if screen.opengl:
                    self.material.texture.OpenGL_texture()
                    ...
                    glVertex2f(cache_localization[0], cache_localization[1])  # bottom left point
                    glTexCoord2f(1.0 + texX, 0.0)
                    glVertex2f(cache_localization[0] + cache_size[0], cache_localization[1])  # bottom right point
                    glTexCoord2f(1.0 + texX, 1.0)
                    glVertex2f(cache_localization[0] + cache_size[0], cache_localization[1] + cache_size[1])  # top right point
                    glTexCoord2f(0.0 + texX, 1.0)
                    glVertex2f(cache_localization[0], cache_localization[1] + cache_size[1])  # top left point
                    glEnd()
            ...
            if not self.border_size == 0:
                ...
                if screen.opengl:
                    ...
                    glColor3f(round((color[0] / 255 * 100) / 100, 4), round((color[1] / 255 * 100) / 100, 4), round((color[2] / 255 * 100) / 100, 4))
                    glBegin(GL_QUADS)
                    glVertex2f(cache_localization[0], cache_localization[1])  # bottom left point
                    glVertex2f(cache_localization[0] + cache_size[0], cache_localization[1])  # bottom right point
                    glVertex2f(cache_localization[0] + cache_size[0], cache_localization[1] + cache_size[1])  # top right point
